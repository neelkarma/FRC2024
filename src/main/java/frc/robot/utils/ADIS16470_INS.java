...
public class ADIS16470_INS implements AutoCloseable, NTSendable {
  ...
  private static enum RegMap {
    ...
    // 1 bit - Data Path Overrun (something internal overran restart the imu and go
    // again)
  ...
  public enum IMUAxis {
    ...
    protected byte[] packetBytes() {
      switch (axis()) {
        case 1:
          return new byte[] { RegMap.X_GYRO_OUT.val(), RegMap.X_GYRO_LOW.val(), // gyro angle
        ...
        case 2:
          return new byte[] { RegMap.Y_GYRO_OUT.val(), RegMap.Y_GYRO_LOW.val(),
        ...
        case 3:
          return new byte[] { RegMap.Z_GYRO_OUT.val(), RegMap.Z_GYRO_LOW.val(),
   ...
   */
  public ADIS16470_INS(IMUAxis rollAxis, IMUAxis pitchAxis, IMUAxis yawAxis, SPI.Port port,
   ...
   * @return 1 if the new calibration time is the same as the current one, 2
   *         error, 0 success
   */
   ...
   * @return 1 if the new yaw axis is the same as the current one, 2 error, else
   *         0.
   */
  ...
  private void readIMU() {
    ...
    // effective size of the buffer if it is to hold the max number of whole
    // datasets
  ...
  /**
   * subfunction of aquire. counts number of full packets waiting in the DMA
   * buffer
   */
  ...
  private Rotation3d bufferReadAng(int[] buffer, int i, double elapsedSamples) {
    double d_roll = (readBuffShort(i, buffer) * MULT_DELTA_ANGLE_SF_16) / elapsedSamples * robotRoll.direction();
    double d_pitch = (readBuffShort(i + 2, buffer) * MULT_DELTA_ANGLE_SF_16) / elapsedSamples * robotPitch.direction();
    double d_yaw = (readBuffInt(i + 4, buffer) * MULT_DELTA_ANGLE_SF_32) / elapsedSamples * robotYaw.direction();
  ...
  private Translation3d bufferReadVel(int[] buffer, int i, double elapsedSamples) {
    double dX = (readBuffInt(i, buffer) * MULT_DELTA_VEL_SF_32) / elapsedSamples * robotRoll.direction();
    double dY = (readBuffShort(i + 4, buffer) * MULT_DELTA_VEL_SF_16) / elapsedSamples * robotPitch.direction();
    double dZ = (readBuffShort(i + 6, buffer) * MULT_DELTA_VEL_SF_16) / elapsedSamples * robotYaw.direction();
